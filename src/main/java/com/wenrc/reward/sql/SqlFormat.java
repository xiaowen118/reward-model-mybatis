...
public class SqlFormat {
	...
	private static class FormatProcess {
		...
		public String perform() {
			...
			while (this.tokens.hasMoreTokens()) {
				...
				if ((this.afterByOrSetOrFromOrSelect) && (",".equals(this.token))) {
					...
					logical();
				} else if (SqlFormat.QUANTIFIERS.contains(this.lcToken)) {
					quantifier();
		...
		private void out() {
			this.result.append(this.token.toUpperCase());
		}

		private void outLowerCase() {
			this.result.append(this.token.toLowerCase());
		}
		...
		private static boolean isFunctionName(String tok) {
			...
			return (isIdentifier) && (!SqlFormat.LOGICAL.contains(tok)) && (!SqlFormat.END_CLAUSES.contains(tok))
	...
	/*public static void main(String[] args) {
		System.out.println(SqlFormat.format(
				"select count(*) from (SELECT *,case enable when 1 then '已启用' else '已停用' end enablestr FROM sys_role WHERE 1=1 limit 0,1) t"));
	}*/
}
